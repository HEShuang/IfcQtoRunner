...
void IfcPreview::parseGeometryStream(Callback_ObjectReady onObjectReady, Callback_ParseFinished onParseFinished)
{
    std::string Prefix("[IfcRunner] ");
    ...
    do{
        ...
        {
            Logger::Notice(Prefix + "same geometry ID, reuse last created meshes");
            spCurrentObject->meshes = spLastCreatedMeshes;

            onObjectReady(spCurrentObject);
            continue;
        }
    ...
    onParseFinished(true, "Parsing geometry completed");
}
